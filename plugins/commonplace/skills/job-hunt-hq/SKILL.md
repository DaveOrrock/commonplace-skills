...
Standard placeholders: `{{roles}}` `{{companies}}` `{{applications}}` `{{profile}}` (criteria + positioning) `{{evidence-locker}}` `{{comp-file}}` `{{contacts}}` `{{debriefs}}` `{{journal}}`.
...
**Company note** (`{{companies}}`): `type: company`, `tier: A | B | C`, plus observations (`- [funding] …`, `- [risk] …`, `- [signal] …`) and relations (`- hiring_for [[role note]]`).
...
| Transition | Skill |
|---|---|
| found → `spotted`/`triaged` (ghost-check, capture) | job-scouting |
| `triaged` → `evaluated` (scored against criteria) | job-evaluation |
| company diligence before deep investment | job-company-research |
| `applying` (CV + letter) | job-cv-tailoring, job-cover-letters |
| `screening`/`interviewing` | job-interview-prep |
| `offer` | job-negotiation |
| weekly metrics, autopsies, pipeline hygiene | job-hunt-review |
| anytime: building the proof base | job-evidence-mining |

## Dashboards

Maintain a HQ note the user glances at daily, built from `aidocs-query` blocks (live tables): active pipeline (`status` not `closed`, sorted by next action), this week's deadlines, offers in flight, funnel counts by status. Add a closed-roles table with `closed_reason` — the review skill reads it.
