---
name: job-evaluation
description: Use when judging whether a role fits the user — before applying, before accepting an interview loop, before an offer decision, or when the user asks "what do you think of this one?", "should I apply?", or seems excited about a specific role. Also when re-evaluating after new information from interviews.
---
...
1. **Dealbreaker screen first.** Check every hard requirement. Any miss is a miss — a great salary cannot buy back a failed non-negotiable. List each verdict explicitly (table). 2+ hard misses → recommendation is *skip*, regardless of everything else.
...
| "It's close enough on most criteria" | Dealbreakers aren't averaged. 2+ hard misses = skip. |
