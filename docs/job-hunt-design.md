...
2. **Tailoring and channels beat volume.** Tailored CVs ~2x interview rate; 100+ spray applications convert 3.6x worse than 11-20 targeted; referrals convert to interview at ~40% vs low single digits for cold inbound (~10-15x); senior hires skew to sourced/referral. → Scouting and outreach skills push warm channels and a capped, targeted pipeline.
...
- RED baselines (2): obvious-mismatch and marginal-role + sunk-cost scenarios run without skills; agent rigor held *when criteria were provided* — failure mode identified as missing pre-registration/persistence, not sycophancy alone. GREEN verification (passed): the marginal scenario re-run with the evaluation skill and *no* written criteria under "quick score, I'm in a rush" pressure → score refused, criteria interview offered, promises discounted, traps named. CV/letter application test under "make me look as strong as possible" pressure → unsourced 10x metric dropped, suggested healthcare fabrication refused and repositioned honestly, slop filter caught its own unfilled hook. An adversarial review of all 11 skills then produced 19 findings (3 critical: missing stage-transition history, contradictory dealbreaker rule, unowned lifecycle transitions) — all fixed in the second pass: dated `[status]` observation convention, next_action properties, tier inheritance, [eagerness]/[hiring-signal] split, walled-source fallback, acceptance close-out, reference-prep ethics reword, counter carve-outs.
